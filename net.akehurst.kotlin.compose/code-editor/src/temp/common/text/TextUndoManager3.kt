 ...
 */
internal class TextUndoManager3(
    ...
    fun undo(state: TextFieldState3) {
        if (!canUndo) {
            return
        }
    ...
    fun redo(state: TextFieldState3) {
        if (!canRedo) {
            return
        }
    ...
    fun record(op: TextUndoOperation) {
        ...
        if (mergedOp != null) {
            ...
            stagingUndo = mergedOp as TextUndoOperation?
            return
        }
    ...
    companion object {
        object Saver : androidx.compose.runtime.saveable.Saver<TextUndoManager3, Any> {
            private val undoManagerSaver = UndoManager.createSaver(TextUndoOperation.Saver)

            override fun SaverScope.save(value: TextUndoManager3): Any {
                return listOf(
            ...
            override fun restore(value: Any): TextUndoManager3? {
                val (savedStagingUndo, savedUndoManager) = value as List<*>
                return TextUndoManager3(
 ...
 *   the stack.
 */
internal fun TextUndoManager3.recordChanges(
